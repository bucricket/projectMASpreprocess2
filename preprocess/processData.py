...
class Landsat(object):
    ...
    def getLC(self, classification):
        ...
        if classification == 'NLCD':  # NLCD
            outfile = os.path.join(self.landsatLC, 'nlcd_2011_landcover_2011_edition_2014_10_10',
...
class ALEXI:
    ...
    def getALEXIdata(self, ALEXIgeodict, isUSA):
        ...
        if not os.path.exists(outfile):
            ...
            ls.clone(mask, et)
            subprocess.check_output('gdal_fillnodata.py %s %s -mask %s -of GTiff' % (outfile2, masked, mask),
                                    shell=True)

            optionList = ['-overwrite', '-s_srs', '%s' % inProj4, '-t_srs', '%s' % self.proj4, \
                          '-te', '%f' % self.ulx, '%f' % self.lry, '%f' % self.lrx, '%f' % self.uly, '-r', 'near', \
                          '-ts', '%f' % self.ncol, '%f' % self.nrow, '-multi', '-of', 'GTiff', '%s' % masked,
                          '%s' % outfile]
            warp(optionList)
...
class MET:
    ...
    def getCFSR(self):
        ...
        pydapURL = os.path.join(ncdcURL, "%s" % self.year, "%d%02d" % (self.year, self.month),
                                "%d%02d%02d" % (self.year, self.month, self.day), hr1file)
        ...
        if os.path.exists(gsip_fn):
            ...
            if not os.path.exists(outFN):
                ...
                outds = gdal.Warp(outFN, in_ds, options=gdal.WarpOptions(resampleAlg='bilinear',
                                                                         dstSRS=self.proj4,
                                                                         outputBounds=(
                                                                         self.ulx, self.lry, self.lrx, self.uly),
                                                                         width=self.ncol,
                                                                         height=self.nrow,
                                                                         multithread=True))
                outds = None
